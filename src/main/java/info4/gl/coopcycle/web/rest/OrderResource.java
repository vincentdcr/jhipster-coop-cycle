...
import info4.gl.coopcycle.service.OrderService;
...
/**
 * REST controller for managing {@link info4.gl.coopcycle.domain.Order}.
 */
@RestController
@RequestMapping("/api")
public class OrderResource {
     ...
     */
    @GetMapping("/orders")
    public List<OrderDTO> getAllOrders(@RequestParam(required = false) String filter) {
        if ("payment-is-null".equals(filter)) {
            log.debug("REST request to get all Orders where payment is null");
            return orderService.findAllWherePaymentIsNull();
        }
